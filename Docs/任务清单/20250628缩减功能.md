...
- **[ ] 架构设计**:
    - **修改确认**: 在架构图中，`get_config_manager` 的流程现在包含调用 `setup_project_paths()`。
- **[ ] 概要设计**:
    - **修改确认**: `PathConfigurationManager` 的职责不变，但其调用时机从初始化过程移至 `setup_project_paths()` 方法内部。
    - **文档更新**: 相关的概要设计文档。
- **[ ] 详细设计**:
    - **修改确认**: 详细描述 `setup_project_paths()` 的实现逻辑，包括如何读取基础配置、调用 `PathConfigurationManager`、以及如何将结果写回 `config.paths`。
    - **文档更新**: 相关的详细设计文档。
- **[ ] 代码实现**:
    - **修改确认**:
...
- **[ ] 需求确认**: 库本身不应再自动创建任何目录。
- **[ ] 架构设计**:
    - **修改确认**: 从架构图中移除"自动目录创建"这一职责。
    - **文档更新**: `Docs/02_architecture/architecture_design.md`
- **[ ] 概要设计**:
    - **修改确认**: `DirectoryCreator` 类可以被移除或其逻辑不再被调用。
    - **文档更新**: 相关的概要设计文档。
- **[ ] 详细设计**:
    - **修改确认**: 明确指出所有 `os.makedirs` 和 `Path.mkdir` 的调用点都将被删除。
    - **文档更新**: 相关的详细设计文档。
- **[ ] 代码实现**:
    - **修改确认**: 全局搜索并删除所有自动创建目录的代码。
    - **涉及文件**: `src/config_manager/core/path_configuration.py`, `src/config_manager/config_node.py`
- **[ ] 测试用例**:
    - **修改确认**: 调整测试用例，在测试代码中显式创建目录来满足测试前置条件，并验证库本身不再创建目录。
    - **涉及文件**: `tests/` 目录下的相关测试文件。
- **[ ] **测试通过状态**: (待执行)
...
- **[ ] 架构设计**:
    - **修改确认**: 在架构图中说明路径自动转换功能的作用域已缩小。
    - **文档更新**: `Docs/02_architecture/architecture_design.md`
- **[ ] 概要设计**:
    - **修改确认**: `set` 方法的逻辑将包含对 `key` 的判断。
    - **文档更新**: 相关的概要设计文档。
- **[ ] 详细设计**:
    - **修改确认**: 详细描述 `ConfigManager.set()` 方法中的 `if key == 'base_dir'` 判断逻辑。
    - **文档更新**: 相关的详细设计文档。
- **[ ] 代码实现**:
    - **修改确认**: 修改 `ConfigManager.set()` 方法，加入对 `key` 的判断逻辑。
...
- **[ ] 架构设计**:
    - **修改确认**: 更新架构图，支持的平台列表只包含 Windows 和 Ubuntu。
    - **文档更新**: `Docs/02_architecture/architecture_design.md`
- **[ ] 概要设计**:
    - **修改确认**: `CrossPlatformPathManager` 的内部逻辑将简化。
    - **文档更新**: 相关的概要设计文档。
- **[ ] 详细设计**:
    - **修改确认**: 明确指出需要从 `SUPPORTED_OS`, `DEFAULT_PATHS` 字典和路径转换逻辑中移除的平台。
    - **文档更新**: 相关的详细设计文档。
- **[ ] 代码实现**:
    - **修改确认**: 在 `CrossPlatformPathManager` 中移除 `macos` 和 `linux` 相关的代码。
    - **涉及文件**: `src/config_manager/core/cross_platform_paths.py`
- **[ ] 测试用例**:
    - **修改确认**: 移除或修改与 `macos`/`linux` 平台相关的测试用例。
...
- **[ ] 详细设计**:
    - **修改确认**: 无需修改。
- **[ ] 代码实现**:
    - **修改确认**: 无需修改，但在重构其他部分时需注意不破坏其功能。
- **[ ] 测试用例**:
    - **修改确认**: 回归测试所有与 `test_mode` 和 `CallChainTracker` 相关的测试用例，确保它们在其他任务完成后依然通过。
    - **涉及文件**: 相关测试文件。
- **[ ] **测试通过状态**: (待执行)

---

### **任务6：简化 `test_mode` 逻辑 (新)**
...
- **[ ] 架构设计**:
    - **修改确认**: 在架构图中，`TestEnvironmentManager` 的职责将大大简化，移除"智能路径替换"。
    - **文档更新**: `Docs/02_architecture/architecture_design.md`
- **[ ] 概要设计**:
    - **修改确认**: `_setup_test_environment` 方法将不再调用 `_replace_all_paths_in_config`。
    - **文档更新**: 相关的概要设计文档。
- **[ ] 详细设计**:
    - **修改确认**: 详细描述 `_setup_test_environment` 的新流程：复制配置后，只覆盖 `base_dir` 的值。
    - **文档更新**: 相关的详细设计文档。
- **[ ] 代码实现**:
    - **修改确认**:
...
- **[ ] 测试用例**:
    - **修改确认**: 此任务的验证方式就是运行完整的 `pytest` 测试套件，确保在移除了所有硬编码路径后，测试仍然能够找到 `src` 模块并成功执行。
- **[ ] **测试通过状态**: (待执行)