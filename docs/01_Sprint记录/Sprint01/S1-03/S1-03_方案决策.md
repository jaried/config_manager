...
| PC-04 | Sprint 状态投影 | RB-08, RB-09 | 时序 28、当前 `master@1ec06a0`、Sprint 校验 | 最终同步/发布前复核 S1-02=`待验收`、S1-03=`方案已决策`、S1-01=`进行中`、总点数 10、规范统计与范围外 dirty 路径均未变化 |
...
- PC-04/Sprint 投影：保持 S1-02=`实施中 / 5`、S1-03=`方案已决策 / 3`、S1-01=`进行中 / 2`，总点数 10，统计 `待办0 | 方案已决策1 | 实施中1 | 进行中1`；其他字段不变。
...
- `backlog_writeback_status=pass`：Sprint01 hash 仍为 `def20d47df618fe53c3fb4b041a10d25e16297c7`；实际值为 S1-03=`方案已决策/P1/3`、S1-02=`实施中/P2/5`、S1-01=`进行中/P1/2`、总点数 10，统计 `待办0/方案已决策1/实施中1/进行中1`，与批准投影完全一致，差异集合为空，故不调用 `update-backlog`。
...
- v6 唯一业务变化是 PC-04：保留 S1-02=`待验收 / 5`，叠加 S1-03=`方案已决策 / 3`，S1-01 保持 `进行中 / 2`；总点数 10，规范统计为 `待办0 | 方案已决策1 | 进行中1 | 待验收1`，其余状态计数为 0，其他字段不变。
...
- PC-04/Sprint 投影：保留 S1-02=`待验收 / 5`，设置 S1-03=`方案已决策 / 3`，S1-01 保持 `进行中 / 2`；总点数 10，规范统计 `待办0 | 方案已决策1 | 进行中1 | 待验收1`，其余状态计数 0。
...
- 当前 Sprint 真源 `master@1ec06a0` 的 S1-02 已为“待验收”；Issue worktree 的旧“实施中”及对应统计在批准后写回中同步为该既有真源。相对当前 Sprint 的最终表差异只剩 S1-03 `待办 -> 方案已决策` 和统计重算，S1-02、S1-01、总点数、优先级、依赖与 Phase 均不变。
...
- 复读结果：Sprint 总点数 10；3 个 Issue；S1-03 方案已决策 1、S1-02 待验收 1、S1-01 进行中 1，其余状态 0；行顺序与表头不变量通过。`decision_backlog_projection_result=pass`。
