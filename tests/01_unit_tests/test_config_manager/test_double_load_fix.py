...
class TestDoubleLoadFix:
    ...
    def test_external_modification_reloads_on_access(self, tmp_path):
        ...
        cfg = get_config_manager(
            str(config_file), watch=True, test_mode=True
        )  # 启用watch以自动重载
    ...
    def test_internal_modification_and_save(self, tmp_path):
        ...
        actual_config_path = (
            cfg._config_path if hasattr(cfg, "_config_path") else config_file
        )
    ...
    def test_reload_after_internal_and_external_changes(self, tmp_path):
        ...
        # 1. Arrange: 初始状态
        create_mock_yaml_file(
            config_file, {"__data__": {"setting": "initial"}, "__type_hints__": {}}
        )
        ...
        create_mock_yaml_file(
            config_file,
            {"__data__": {"setting": "external_final"}, "__type_hints__": {}},
        )
