...
class TestConfigFilePath:
    ...
    def test_tc0010_001_001_config_file_path_storage(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert stored_path == config_file, (
                f"存储的路径应该匹配，期望: {config_file}, 实际: {stored_path}"
            )
            assert retrieved_path == config_file, (
                f"检索的路径应该匹配，期望: {config_file}, 实际: {retrieved_path}"
            )
            assert actual_path == config_file, (
                f"实际路径应该匹配，期望: {config_file}, 实际: {actual_path}"
            )

            # 验证路径信息在重载后仍然存在
            cfg.reload()
            reloaded_path = cfg.get_config_file_path()
            assert reloaded_path == config_file, (
                f"重载后路径应该保持一致，期望: {config_file}, 实际: {reloaded_path}"
            )
    ...
    def test_tc0010_001_003_config_file_path_persistence(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cfg2 = get_config_manager(
                config_path=config_file, auto_create=False, autosave_delay=0.1
            )

            # 验证路径信息被正确加载
            second_path = cfg2.get_config_file_path()
            assert second_path == config_file, (
                f"重新加载后路径应该一致，期望: {config_file}, 实际: {second_path}"
            )
    ...
    def test_tc0010_001_004_config_file_path_attribute_access(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # 验证两种访问方式结果一致
            assert method_path == attribute_path, (
                f"方法访问和属性访问应该一致，方法: {method_path}, 属性: {attribute_path}"
            )
            assert method_path == config_file, (
                f"路径应该匹配，期望: {config_file}, 实际: {method_path}"
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert os.path.isabs(stored_path), (
                    f"存储的路径应该是绝对路径: {stored_path}"
                )
                assert stored_path == expected_absolute_path, (
                    f"绝对路径应该正确，期望: {expected_absolute_path}, 实际: {stored_path}"
                )
    ...
    def test_tc0010_001_006_config_file_path_consistency_after_operations(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # 批量更新后
            cfg.update({"batch_test": "batch_value", "nested": {"key": "value"}})
    ...
    def test_tc0010_001_007_config_file_path_in_yaml_structure(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cfg.nested_data = {"level1": {"level2": "deep_value"}}
            time.sleep(0.2)  # 等待自动保存

            # 直接读取YAML文件验证结构
            with open(config_file, "r", encoding="utf-8") as f:
                yaml_content = load_yaml(f.read())

            # 验证YAML文件结构
            assert "__data__" in yaml_content, "YAML文件应该包含__data__节点"
            data_section = yaml_content["__data__"]

            assert "config_file_path" in data_section, (
                "数据节点应该包含config_file_path字段"
            )
            assert data_section["config_file_path"] == config_file, (
                "YAML中的路径应该正确"
            )
