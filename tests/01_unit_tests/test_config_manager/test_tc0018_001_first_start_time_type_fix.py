...
def test_first_start_time_type_consistency():
    ...
    assert isinstance(config.first_start_time, datetime), (
        f"first_start_time 应该是 datetime 对象，实际是 {type(config.first_start_time)}"
    )

    # 验证时间值是否正确
    assert config.first_start_time == test_time, (
        f"first_start_time 值不正确: {config.first_start_time} != {test_time}"
    )
...
def test_first_start_time_yaml_type_hints():
    ...
    # 验证类型注释中包含 first_start_time 的类型注释
    assert hasattr(config, "_type_hints"), "配置中应该包含 _type_hints 属性"

    type_hints = config._type_hints
    print(f"类型注释: {type_hints}")

    # 验证 first_start_time 的类型注释
    assert "first_start_time" in type_hints, "类型注释中应该包含 first_start_time"
    assert type_hints["first_start_time"] == "datetime", (
        f"first_start_time 类型注释应该是 'datetime'，实际是 {type_hints['first_start_time']}"
    )
...
def test_first_start_time_reload_with_type_hints():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # 重新加载配置（使用相同的路径）
        new_config = get_config_manager(config_path=config_path, test_mode=True)
        ...
        # 验证从 YAML 加载的时间类型
        assert isinstance(new_config.first_start_time, datetime), (
            f"从 YAML 加载的 first_start_time 应该是 datetime 对象，实际是 {type(new_config.first_start_time)}"
        )

        # 验证时间值是否正确
        assert new_config.first_start_time == test_time, (
            f"从 YAML 加载的时间值不正确: {new_config.first_start_time} != {test_time}"
        )
...
def test_first_start_time_string_conversion():
    ...
    assert isinstance(config.first_start_time, datetime), (
        f"字符串时间应该被转换为 datetime 对象，实际是 {type(config.first_start_time)}"
    )

    # 验证时间值是否正确
    expected_time = datetime.fromisoformat(time_str)
    assert config.first_start_time == expected_time, (
        f"转换后的时间值不正确: {config.first_start_time} != {expected_time}"
    )
