...
class TestTC0012006ProtectedFields:
    ...
    def _create_mock_config(self) -> dict:
        ...
        return {
            ...
            "url_validation": {
                ...
                "exclude_image_patterns": [r"\.jpg$", r"\.png$", r"\.gif$"],
            },
            # 真正的路径字段
    ...
    def test_tc0012_006_001_network_urls_not_replaced(self, mock_load_config):
        """测试网络URL不被路径替换"""
        # 准备
    ...
    @patch("src.config_manager.core.file_operations.FileOperations.load_config")
    def test_tc0012_006_002_http_headers_not_replaced(self, mock_load_config):
    ...
    @patch("src.config_manager.core.file_operations.FileOperations.load_config")
    def test_tc0012_006_003_regex_patterns_not_replaced(self, mock_load_config):
        """测试正则表达式模式不被路径替换"""
        # 准备
        mock_config = self._create_mock_config()
        mock_load_config.return_value = {"__data__": mock_config, "__type_hints__": {}}
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            # 验证正则表达式没有被替换
            assert (
                config_manager.url_validation.level2_pattern
                == r"^https?://[^/]+/chapter/\d+$"
            )
...
""")
        ...
        try:
            ...
            assert config_manager.base_dir != "/tmp/original/base/path", (
                f"base_dir应该被替换，但仍然是: {config_manager.base_dir}"
            )

            # 验证替换后的路径包含测试环境路径
            # 标准化路径格式进行比较，处理Windows路径的反斜杠和正斜杠混合问题
            normalized_base_dir = (
                str(config_manager.base_dir).replace("\\", "/").replace("//", "/")
            )

            # 检查路径包含测试环境特征
            assert (
                "temp" in normalized_base_dir.lower()
                or "tmp" in normalized_base_dir.lower()
            ), f"base_dir应该包含temp或tmp: {normalized_base_dir}"
            assert "tests" in normalized_base_dir, (
                f"base_dir应该包含tests: {normalized_base_dir}"
            )

            # work_dir会因为向后兼容性自动同步paths.work_dir，所以会被测试模式替换
            # 验证work_dir被替换到测试环境
            assert config_manager.paths.work_dir != "/tmp/original/work/path", (
                f"work_dir应该因向后兼容而被替换，但仍然是: {config_manager.paths.work_dir}"
            )
            assert "tests" in config_manager.paths.work_dir, (
                f"work_dir应该包含tests路径: {config_manager.paths.work_dir}"
            )
            ...
            assert config_file.startswith(tempfile.gettempdir()), (
                f"禁止删除非临时文件: {config_file}"
            )
...
""")
            ...
            assert (
                config_manager.headers.User_Agent == "Mozilla/5.0 (compatible; Bot/1.0)"
            )
