...
class TestTC0012004BackupIsolation:
    ...
    def test_tc0012_004_002_backup_file_creation_isolation(self):
        ...
        cfg1 = get_config_manager(
            first_start_time=fixed_time1, autosave_delay=0.1, test_mode=True
        )
        ...
        cfg2 = get_config_manager(
            test_mode=True, first_start_time=fixed_time2, autosave_delay=0.1
        )
    ...
    def test_tc0012_004_003_backup_directory_structure(self):
        ...
        test_cfg = get_config_manager(
            test_mode=True, first_start_time=fixed_time, autosave_delay=0.1
        )
    ...
    def test_tc0012_004_004_multiple_test_instances_backup_isolation(self):
        ...
        # 创建两个不同时间的测试实例
        test_cfg1 = get_config_manager(
            test_mode=True, first_start_time=time1, autosave_delay=0.1
        )
        test_cfg2 = get_config_manager(
            test_mode=True, first_start_time=time2, autosave_delay=0.1
        )
    ...
    def test_tc0012_004_006_backup_with_custom_config_path(
        self, mock_exists, mock_find_root
    ):
        """测试自定义配置路径下的备份隔离"""
        # 设置Mock
        ...
        # 使用自定义配置路径的测试模式
        with patch("builtins.open"), patch("os.makedirs"), patch("shutil.copy2"):
            test_cfg = get_config_manager(
