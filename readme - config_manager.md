...
一个强大、易用的 Python 配置管理库，支持自动保存、类型提示、文件监视、**安全 YAML codec**、**测试模式**等高级功能。
...
- [高级功能](#高级功能)
  ...
  - [YAML数据语义管理](#6-yaml数据语义管理) 🛡️
- [配置文件格式](#配置文件格式)
  - [YAML codec 与数据语义](#yaml-codec-与数据语义) 🛡️
- [测试中的配置管理](#测试中的配置管理)
  - [测试模式功能](#测试模式功能) 🧪
  - [测试环境管理](#测试环境管理)
- [完整示例](#完整示例)
...
配置管理器通过 PyYAML>=6.0 的安全加载和导出路径处理普通 Python 数据树：

- 支持范围内的标量、序列和映射在加载、保存后保持类型和值语义等价。
- 拒绝 unsafe tag、重复 key、未声明或不支持的数据类型。
- raw YAML 可以直接加载；首次保存时规范化为包含 `__data__` 与 `__type_hints__` 的标准包络。
...
配置管理器使用 PyYAML>=6.0 的集中式安全 codec。保存契约保证支持范围内的数据类型和值语义，不保证文本或对象图表达保真：

- raw YAML 首次保存会规范化为 `__data__` 与 `__type_hints__` 标准包络。
- 注释、空行、缩进、引号和 flow/block 排版可能变化或丢失。
- anchor/alias 名称及共享引用表达不属于保真契约。
- unsafe tag、重复 key 和不支持的声明类型会被拒绝，不会通过兼容回退绕过验证。
...
A: 可能。注释、排版、引号样式以及 anchor/alias 表达不属于保存契约，可能被规范化或丢失。配置管理器只保证支持范围内 YAML 数据的类型和值语义；raw YAML 首次保存会规范化为 `__data__` 与 `__type_hints__` 标准包络。
...
- ✨ **新增**：集中式安全 YAML codec
  - 支持普通 Python 标量、序列和映射的数据语义
  - raw YAML 首次保存时规范化为标准包络
  - 拒绝 unsafe tag、重复 key 和不支持的数据类型
- 🔧 **改进**：候选数据验证与原子替换
  - 保存前验证候选文件能够安全加载且数据语义等价
  - 候选验证成功后才原子替换正式配置
- 🧪 **测试**：增强多线程测试稳定性
  - 优化多线程环境下的输出捕获
  - 改进测试用例的健壮性
- 📚 **文档**：明确数据语义与文本保真的边界
  - 说明注释、排版、anchor/alias 表达不属于保真契约
  - 添加安全 codec 使用边界

**迁移说明**：
- 依赖统一为 `PyYAML>=6.0`
- 公开 ConfigManager/FileOperations 签名保持兼容
- raw YAML 可加载，首次保存时规范化为标准包络
